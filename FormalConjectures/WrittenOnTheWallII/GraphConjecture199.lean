...
/-- For a single-vertex graph (`Fin 1`), the vertex connectivity is 0
by definition (since `Fintype.card (Fin 1) = 1 ≤ 1`). -/
@[category test, AMS 5]
example (G : SimpleGraph (Fin 1)) : vertexConnectivity G = 0 := by
  unfold vertexConnectivity
  simp

/-- The vertex connectivity of `K₃` is at most 2: removing both non-trivial vertices
(leaving a single vertex or empty set) satisfies `S.card = Fintype.card (Fin 3) - 1 = 2`. -/
@[category test, AMS 5]
example : vertexConnectivity (⊤ : SimpleGraph (Fin 3)) ≤ 2 := by
  unfold vertexConnectivity
  simp only [Fintype.card_fin, show ¬ 3 ≤ 1 from by decide, if_false]
  apply Nat.sInf_le
  -- Exhibit S = {0, 1} with card 2; then S.card = 3 - 1 = 2
  exact ⟨{0, 1}, by decide, Or.inr (by decide)⟩
